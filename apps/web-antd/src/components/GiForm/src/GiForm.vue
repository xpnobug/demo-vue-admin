<template>
  <a-form ref="formRef" :auto-label-width="true" :model="modelValue" v-bind="options.form">
    <a-row :gutter="8" :collapsed="collapsed" class="w-full" v-bind="options.grid">
      <template v-for="(item, index) in columns" :key="item.field">
        <a-col
          v-if="!isHide(item.hide)" v-show="colVShow(index)"
          v-bind="item.gridItemProps || props.options.gridItem"
        >
<!--        <a-col-->
<!--          v-if="!isHide(item.hide)" v-show="colVShow(index)"-->
<!--          :span="item.span?.xs || options.gridItem?.span?.xs"-->
<!--          :xs="item.span?.xs"-->
<!--          :sm="item.span?.sm"-->
<!--          v-bind="item.gridItemProps || props.options.gridItem"-->
<!--        >-->
          ...
          >
            ...
            >
              ...
              <template v-else-if="item.type === 'switch'">
                <component
                  :is="`a-${item.type}`"
                  v-model:checked="modelValue[item.field as keyof typeof modelValue]"
      ...
      <a-col v-if="!options.btns?.hide" :suffix="options.fold?.enable">
        <a-space :size="[8, 16]" style="flex-wrap: nowrap" wrap>
          <slot name="suffix">
            ...
            <a-button
              v-if="options.fold?.enable" class="gi-form__fold-btn" size="mini" type="text"
